...
above, because the main fixtures are generated overlay-free, so the Etymology section never appears in
them. `etym/` holds a small, hand-authored test overlay (`etym/etym.onym`, deliberately not from any
Wiktionary dump so it never churns), a `etym/corpus.txt`, and UTF-8 `etym/fixtures/`. The check lives
in `crates/onym-engine/tests/etymology.rs`: it opens an engine over WordNet plus the test overlay,
dumps each corpus word, and compares as UTF-8. Regenerate the fixtures
